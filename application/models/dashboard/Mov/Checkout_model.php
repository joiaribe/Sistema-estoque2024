...
class CheckoutModel extends HTMLCheckoutModel {
     ...
     * @var String
     */
    var $location_error = 'Mov/products';
    ...
    private function InsertOnDatabase() {
        ...
        foreach ($id as $id_product) {
            $amount = $qnt[$id_product];
            $valor = Func::negativeToPositive($value[$id_product]);

            // Check Expense, in this case is commissioner of service
            $Ex = !($expense[$id_product] == 0) ? $this->InsertExpense($expense[$id_product], $id_product, $amount) : true;

            // Earning
            $EA = $this->InsertEarnings($valor, $id_product, $amount);
            // Receipt itens
            $ri = $this->InsertReceiptItens($id_product, $amount);
        }
    ...
    private function CheckErros($Ex, $EA, $r1, $ri, $ExP, $plot) {
        ...
        switch (true) {
            case (!$Ex):
                Call_JS::alerta('Ocorreu um erro ao inserir a saida do produto');
                Call_JS::retornar($erro);
                die;
                break;
            case (!$EA):
                Call_JS::alerta('Ocorreu um erro ao inserir a entrada do produto');
                Call_JS::retornar($erro);
                die;
                break;
            case (!$r1 || !$ri):
                Call_JS::alerta('Ocorreu um erro ao inserir o recibo do produto');
                Call_JS::retornar($erro);
                die;
                break;
            case (!$ExP):
                $msg = ' Ocorreu um erro ao inserir o valor do desconto da saida do produto !';
                Call_JS::alerta($msg);
                Call_JS::retornar($erro);
                die($msg);
                break;
            case (!$plot):
                ...
                break;
            default:
                Call_JS::alerta('Nova Venda adicionada com sucesso !');
    ...
    private function GeneratePlots() {
        ...
        for ($i = 1; $i <= $plots; $i++) {
            ...
            $q = new Query;
            $q
                    ->insert_into('input_product_plots', array(
                        'id_receipt' => self::GetParam('IDR'),
                        'plot' => $i,
                        'plot_value' => $value
                            )
     ...
     * @return boolean
     */
    private function InsertExpense($expense, $id_product, $qnt) {
        if ($expense !== 0) {
            $date = $this->format_date($_POST['horario']);
            $name = Func::array_table($this->table_main, array('id' => $id_product), 'nome');
            $descri = Func::array_table($this->table_main, array('id' => $id_product), 'descri');
            $s = $_POST['status'];
            $status = isset($s) ? true : false;
            $id_client = $_POST['client'] == 'nothing' ? NULL : $_POST['client'];
            $q = new Query;
     ...
     * @return boolean
     */
    private function InsertExpenseAvista($valor, $id_product, $qnt) {
        $date = $this->format_date($_POST['horario']);
        $name = Func::array_table($this->table_main, array('id' => $id_product), 'nome');
        $descri = Func::array_table($this->table_main, array('id' => $id_product), 'descri');
        $s = $_POST['status'];
     ...
     * @return boolean
     */
    private function InsertExpenseParcelado($id_product, $qnt) {
        $date = $this->format_date($_POST['horario']);
        $name = Func::array_table($this->table_main, array('id' => $id_product), 'nome');
        $descri = Func::array_table($this->table_main, array('id' => $id_product), 'descri');
        $s = $_POST['status'];
     ...
     * @return boolean
     */
    private function InsertExpenseParcelaPlusEntry($valor, $id_product, $qnt) {
        $date = $this->format_date($_POST['horario']);
        $name = Func::array_table($this->table_main, array('id' => $id_product), 'nome');
        $descri = Func::array_table($this->table_main, array('id' => $id_product), 'descri');
        $s = $_POST['status'];
     ...
     * @return boolean
     */
    private function InsertExpensePercentPlusEntry($valor, $id_product, $qnt) {
        $date = $this->format_date($_POST['horario']);
        $name = Func::array_table($this->table_main, array('id' => $id_product), 'nome');
        $descri = Func::array_table($this->table_main, array('id' => $id_product), 'descri');
        $s = $_POST['status'];
     ...
     * @return boolean
     */
    private function InsertEarnings($valor, $id_product, $qnt) {
        $metthod = self::GetParam('type_metthod');
        switch ($metthod) {
            case 'parcelado':
                $this->InsertExpenseParcelado($id_product, $qnt);
                break;
            case 'entrada_e_parcelas':
                $this->InsertExpenseParcelaPlusEntry($valor, $id_product, $qnt);
                break;
            case 'porcentagem_e_Parcelas':
                $this->InsertExpensePercentPlusEntry($valor, $id_product, $qnt);
                break;
            default:
                $this->InsertExpenseAvista($valor, $id_product, $qnt);
                break;
        }
        return true;
    }
     ...
     * @return Void
     */
    private function InsertReceiptItens($id_product, $qnt) {
        $brute = Func::_sum_values($this->table_main, 'valor', array('id' => $id_product)) * $qnt;
        $discount = Func::array_table($this->table_output, array('id_product' => $id_product, 'discount' => true), 'value');
        $despesa = Func::_sum_values($this->table_output, 'value', array('id_product' => $id_product, 'discount' => false));
        ...
        $this->diminuir_estoque($id_product, $qnt);
        return true;
    }

}

class HTMLCheckoutModel {

    /**
     * Table main where has all information needed
     * @var string 
     */
    var $table_main = 'produtos';

    /**
     * Table ouput
     * @var string 
     */
    var $table_output = 'output_product';

    /**
     * Table input
     * @var string 
     */
    var $table_input = 'input_product';

    /**
     * Recipt Enum column
     * @var string 
     */
    var $ReciptEnum = 'Products';
    ...
    public function __construct() {
        ...
        if (($product_list !== false) && $param !== 'Insert') {
            ...
            Call_JS::retornar(URL . 'dashboard/Mov/products');
            die(' Carrinho vázio !');
        }

        // check stock before insert new receipt
        $this->CheckStock();
    ...
    private function check_form_payment(array $data, $brute, $total) {
        switch ($data['type_metthod']) {
            ...
            case 'entrada_e_parcelas':
                ...
                break;
            case 'porcentagem_e_Parcelas':
                $value = ($total * $data['percent_entry']) / 100;
...
EOF;
    ...
    private function LoopProducts($value = NULL, $return = false) {
        ...
        if (!empty($product_list)) {
            ...
            foreach ($product_list as $product) {
                ...
                // get info of product
                $product_price = Func::array_table($this->table_main, array('id' => $product_id), 'valor');
                $product_despe = Func::array_table($this->table_main, array('id' => $product_id), 'valor_original');
                $product_despe_check = Func::array_table($this->table_main, array('id' => $product_id), 'autoexpense');
                // check automatic expense is true
                if ($product_despe_check == true) {
                    $product_despe = 0;
                } else {
                    $product_despe = $product_despe - ($product_qty * $product_price);
                }

                $product_amount[1] = $product_price * $product_qty;
                $product_amount[2] = $product_despe - ($product_price * $product_qty);
                $product_amount[3] = $product_despe * $product_qty;

                $product_amount[4] = $product_qty * $product_price;
                $valor = $product_amount[4] + $valor;
                // negative number
                $lucro = $valor - ($product_despe * -1);
                // positive
                $positive_l = $lucro * -1;
                // positive number
                $des = $product_despe * -1;
                $result .= '	
                    <!-- Cart item -->
                        <tr>
                            <td>' . $i++ . '</td>
                            <td>
                                <h4>' . Func::array_table($this->table_main, array('id' => $product_id), 'nome') . '</h4>
                                <p>' . Func::array_table($this->table_main, array('id' => $product_id), 'descri') . '</p>
                            </td>
                            <td class="text-center">R$ ' . number_format($product_price, 2, ',', '.') . '</td>
                            <td class="text-center">' . $product_qty . '</td>
                            <td class="text-center">R$ ' . number_format($product_amount[1], 2, ',', '.') . '</td>
                        </tr>
                        <input type="hidden" value="' . $positive_l . '" name="value[' . $product_id . ']">   
                        <input type="hidden" value="' . $des . '" name="expense[' . $product_id . ']">
    ...
    private function CheckStock() {
        ...
        if (!empty($product_list)) {
            foreach ($product_list as $product) {
                ...
                // check stock
                $this->check_qnt($product_id, $product_qty);
            }
        }
    }

    /**
     * reduces inventory in accordance with the amount
     * @access protected
     * @param integer $id ID of product
     * @param integer $qnt total amout of product
     * @return Boolean
     */
    protected function diminuir_estoque($id, $qnt) {
        $q = new Query;
        $q
                ->update($this->table_main)
                ->set(
                        array(
                            'quantidade' => ($this->pegar_quantidade($id) - $qnt)
                        )
                )
                ->where_equal_to(
                        array(
    ...
    /**
     * takes stock of the product
     * @access protected
     * @param $id ID of Product
     * @return integer
     */
    protected function pegar_quantidade($id) {
        $q = new Query;
        $q
                ->select()
                ...
                ->run();
        if ($q) {
            $dados = $q->get_selected();
            return $dados['quantidade'];
        }
    }

    /**
     * checks if the stock is greater than the selected distance
     * @access protected
     * @param integer $id ID of product
     * @param integer $qnt total amout of product
     * @return Boolean
     */
    protected function verify_quantidade($id, $qnt) {
        return (($this->pegar_quantidade($id) - $qnt) < 0) ? TRUE : FALSE;
    }

    /**
     * This method checks can be inserted, otherwise show message, redirenciona and interrupts.
     * @access protected
     * @param integer $id ID of product
     * @param integer $qnt total amout of product
     * @return void
     */
    protected function check_qnt($id, $qnt) {
        if ($this->verify_quantidade($id, $qnt) == TRUE) {
            Call_JS::alerta(' Você não pode vender, porque seu estoque é ' . $this->pegar_quantidade($id) . ' unidades e você tentou vender ' . $qnt . ' unidades');
            Call_JS::retornar(URL . 'dashboard' . DS . $this->location_error);
            exit();
        }
    }
...
EOF;
    ...
    private function getExpenseValue($discount, $ExpenseValue, $brute, $return = NULL) {
        ...
        if ($return !== NULL) {
            ...
            $title = 'Despesa de produtos. Valor atual com a despesa R$ ' . \Func::FloatToReal($now);
            $name = 'Despesas';
...
EOF;
    ...
    private function HTML() {
        $data = strftime('%d de %B, %Y %H:%M', strtotime(filter_input(INPUT_POST, 'horario')));
...
<div class="row">
    <div class="col-md-12">
        ...
        <form action="{$url}dashboard/Mov/Checkout/Insert" method="post">
                ...
                <div class="text-center invoice-btn">
                    ...
                    <a href="{$url}dashboard/Mov/products" class="btn btn-danger btn-lg"><i class="fa fa-times"></i> Cancelar </a>
                </div>
            </form>          
            </div>
        </section>
