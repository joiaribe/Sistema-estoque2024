 ...
 */
class ServicesHTML extends requireds {
...
EOF;
    ...
    private function _make_list_mode_tables(array $param) {
        $name = GetInfo::_name($param['id_poster']);
    ...
    private function loop_users($data = false) {
        ...
        if ($q) {
            ...
            foreach ($users as $user) {
                if ($data !== false) {
                    $result.= '<option value="' . $user['id'] . '"' . $this->_check_selected($user['id'], $data) . '>' . \GetInfo::_name($user['id_poster']) . '</option>';
                } else {
                    $result.= '<option value="' . $user['id'] . '">' . \GetInfo::_name($user['id_poster']) . '</option>';
                }
            }
            return $result;
        }
 ...
 */
class requireds extends ServicesConfig {
