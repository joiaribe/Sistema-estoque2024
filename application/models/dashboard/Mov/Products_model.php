...
class ProductsModel extends ProductsHTML {
...
EOF;
    ...
    private function verify_pic_product($picture) {

        return $picture == NULL ? URL . 'public/images/produtos/no-image.jpg' : URL . 'public/images/produtos/' . $picture;
    }

    private function loop_products() {
        $q = new Query;
        $q
                ->select()
                ->from('produtos')
        ...
        if ($q) {
            ...
            foreach ($data as $dados) {
                $result.= ' <input type="hidden" pimage="' . $this->verify_pic_product($dados['foto']) . '" pprice="' . $dados['valor'] . '" pdesc="Em Estoque : ' . $dados['quantidade'] . '" pcategory="' . Func::array_table('marcador', array('id' => $dados['marcador']), 'nome') . '" pname="' . Func::str_truncate($dados['nome'], 60) . '" pid="' . $dados['id'] . '">';
            }
     ...
     * @return string
     */
    private function loop_clientes() {
        $q = new Query;
        $q
                ->select()
                ->from('clientes')
...
<div class="form-group">
    ...
    <div class="col-md-6">
        ...
        <option value="nothing" selected>--Nenhum--</option>
        {$this->loop_clientes()}
        </select>                               
    </div>
</div>
...
EOF;
    ...
    private function BuildForm() {
        $url = URL;
        return <<<EOF
<form class="cmxform form-horizontal" id="checkout" method="post" enctype="multipart/form-data" action="{$url}dashboard/Mov/Checkout">
    <div class="form-group">
        <div class="col-md-11">
            <!-- Smart Cart HTML Starts -->
            <div id="SmartCart" class="scMain">{$this->loop_products()}</div>
        </div>
    </div>

    <div class="col-md-5">{$this->LeftElements()}</div>

    <div class="col-md-5">{$this->RightElements()}</div>
    </div>
</form>
EOF;
    }
