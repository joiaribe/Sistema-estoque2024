 ...
 */
class MarkerHTML extends requireds {
    ...
    protected function verify_qnt($qnt) {
        if ($qnt > 0) {
            return $qnt == 1 ? '1 Produto' : $qnt . ' Produtos ';
        } else {
            return '<span class="badge bg-important">Não Encontrado</span>';
        }
    }
    ...
    protected function contain_table(array $fetch) {
        ...
        $total = \Func::_contarReg('produtos', $where);
        $data = strftime('%d de %B, %Y &aacute;s %H:%M:%S', strtotime($fetch['data']));
        return <<<EOFPAGE
<tr class="gradeX">
    <td class="center uniformjs"><input type="checkbox" name="checkbox[]" value="{$fetch['id']}" /></td>
    <td>{$fetch['nome']}</td>
    <td>{$this->verify_qnt($total)}</td>
    <td>{$data}</td>
...
EOFPAGE;
    ...
    private function tools_call(array $ids) {
        ...
        if ($q) {
            $data = $q->get_selected();
            foreach ($data as $dados) {
                $result.= '<li><a title="' . $this->msg['manager'] . ' ' . $dados['name'] . '" href="' . URL . $dados['link'] . '">' . $this->msg['manager'] . ' ' . $dados['name'] . '</a></li>';
            }
            return $result;
        }
    }
    ...
    protected function make_tools() {
        ...
        $return.= $this->tools_call($this->ids_tools) . '
                                    <li class="divider"></li>
 ...
 */
class requireds extends MarkerConfig {
