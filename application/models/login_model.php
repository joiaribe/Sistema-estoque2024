...
class LoginModel {
    ...
    public function registerNewUser() {
        ...
        if (!$this->checkCaptcha()) {
            ...
            $date = new DateTime();
            $user_creation_timestamp = $date->getTimestamp();
    ...
    public function registerNewUserWithFacebook($facebook_user_data) {
        ...
        $clean_user_name_from_facebook = $facebook_user_data["first_name"];

        $q = $this->db->prepare("SELECT user_id FROM users WHERE user_name = :clean_user_name_from_facebook");
        $q->execute(array(':clean_user_name_from_facebook' => $clean_user_name_from_facebook));

        if ($q->rowCount() >= 1) {
            $clean_user_name_from_facebook = $this->generateUniqueUserNameFromExistingUserName($clean_user_name_from_facebook);
        }
