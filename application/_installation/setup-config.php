...
class SetupConfig extends SetupConfigHTML {
    ...
    private function MakeFile() {
        ...
        if ($this->CheckConnection($_POST) == true) {
            ...
            $this->WriteUTF8File($this->sql, $this->ReplaceASCIIFwrite($this->MakeSQLFile($_POST)));
            $this->WriteUTF8File('../../.htaccess', $this->makehtaccess($_POST));
            // defines the permission to 666
            chmod($file, 0666);
            chmod($this->sql, 0666);
            chmod('../../.htaccess', 0666);
            // makes a final check, if the file was created
            if (file_exists($file) && file_exists($this->sql))
