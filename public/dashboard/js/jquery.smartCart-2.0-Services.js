...
(function ($) {
    $.fn.smartCart = function (options) {
        ...
        return this.each(function () {
            ...
            // Labels & Messages              
            var labelCartMenuName = 'Carrinho (_COUNT_)';  // _COUNT_ will be replaced with cart count
            var labelCartMenuNameTooltip = "Carrinho | Total de Serviços: _PRODUCTCOUNT_ | Quantidade Total: _ITEMCOUNT_";
            var labelProductMenuName = 'Serviços';
            var labelSearchButton = "Buscar";
            var labelSearchText = "Consulta";
            var labelCategoryText = "Marcador";
            var labelClearButton = "Limpar";
            var labelAddToCartButton = "Adicionar";
            var labelQuantityText = "Qnt";
            var labelProducts = 'Serviços';
            var labelPrice = 'Valor';
            var labelSubtotal = 'Subtotal';
            var labelTotal = 'Total';
            var labelRemove = 'Remover';
            var labelCheckout = 'Confirmar';

            var messageConfirmRemove = 'Você quer remover "_PRODUCTNAME_" do carrinho?'; //  _PRODUCTNAME_ will be replaced with actula product name
            var messageCartEmpty = "Carrinho Vazio";
            var messageProductEmpty = "Não há Serviços para mostrar";
            var messageProductAddError = "O Serviço não pode adicionar";
            var messageItemAdded = 'O Serviço foi adicionado ao carrinho';
            var messageItemRemoved = 'Serviço removido do carrinho';
            var messageQuantityUpdated = 'Quantidade Serviço atualizada';
            var messageQuantityErrorAdd = 'Quantidade inválida. Não foi possivel adicionar o Serviço';
            var messageQuantityErrorUpdate = 'Quantidade inválido. Quantidade não é possível atualizar';
            ...
            if (options.enableSearch) {
                var btSearch = $('<a>' + labelSearchButton + '</a>').attr("href", "#").addClass("scSearch").attr("title", "Buscar Produto");
                $(btSearch).click(function () {
                    var searcStr = $(txtSearch).val();
                    populateProducts(searcStr);
                    return false;
                });
                var btClear = $('<a>' + labelClearButton + '</a>').attr("href", "#").addClass("scSearch").attr("title", "Limpar Busca");
            ...
            var btCheckout = $('<a>' + labelCheckout + '</a>').attr("href", "#").addClass("scCheckoutButton btn btn-success");
            ...
            function addCartItemDisplay(objProd, Quantity) {
                ...
                var btRemove = $('<a>' + labelRemove + '</a>').attr("rel", pIndex).attr("href", "#").addClass("scRemove").attr("title", "Remover do carrinho");
            ...
            function removeFromCart(idx) {
                ...
                if (confirm(removeMsg)) {
                    ...
                    $("#divCartItem" + idx, elmCartList).slideUp("slow", function () {
                        ...
                        $.gritter.add({
                            text: '<div class="alert alert-success alert-block fade in"><button data-dismiss="alert" class="close close-sm gritter-close" type="button"><i class="fa fa-times"></i></button> <strong>Sucesso !</strong> ' + messageItemRemoved + '</div>',
                            sticky: true
                        });
            ...
            function populateProducts(searchString) {
                ...
                $(products).each(function (i, n) {
                    ...
                    if (isValid && isCategoryValid) {
                        ...
                        var btAddToCart = $('<a>' + labelAddToCartButton + '</a>').attr("href", "#").attr("rel", i).attr("title", labelAddToCartButton).addClass("scAddToCart btn btn-primary");
    ...
    $.fn.smartCart.defaults = {
        ...
        productItemTemplate: '<strong><%=pname%></strong><br /><strong>Descrição:</strong> <%=pdescri%><br /><strong>Comissão:</strong> <%=pcomi%><small><%=pdesc%></small><br /><strong>Preço: <%=pprice%></strong>',
